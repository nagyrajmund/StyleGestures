...
from motion_features import extract_joint_angles_saga, extract_hand_pos, extract_style_features
...
def cut_audio(filename, timespan, destpath, starttime=0.0, endtime=-1.0):
    ...
    while (starttime+timespan) <= endtime:
        ...
        wav_outfile = join(destpath, out_basename + "_" + str(suffix).zfill(3) + '.mov.wav')
...
def cut_bvh(filename, timespan, destpath, starttime=0.0, endtime=-1.0):
    ...
    while (starttime+timespan) <= endtime:
        out_basename = os.path.splitext(os.path.basename(filename))[0]
        bvh_outfile = join(destpath, out_basename + "_" + str(suffix).zfill(3) + '.bvh')
...
def import_data(file, motion_path, speech_data, style_path, mirror=False, start=0, end=None):
    ...
    motion_data = np.load(join(motion_path, file + suffix + '.npz'))['clips'].astype(np.float32)        
    n_motion_feats = motion_data.shape[1]

    speech_data = np.load(join(speech_path, file + '.npy')).astype(np.float32)
    
    if style_path is not None:
        style_data = np.load(join(style_path, file + suffix + '.npy')).astype(np.float32)
...
if __name__ == "__main__":
    ...
    data_root = '../data/SAGA/raw'
    processed_dir = '../data/SAGA/processed'
    bvhpath = join(data_root, 'Motion')
    audiopath = join(data_root, 'Audio')
    held_out = ['V25']
    ...
    path = join(processed_dir, f'features_{fps}fps')
    motion_path = join(path, motion_feat)
    speech_path = join(path, speech_feat)
    hand_path = join(path, 'hand_pos')
    vel_path = join(path, 'MG-V')
    radius_path = join(path,'MG-R')
    rh_path = join(path, 'MG-H')
    #lh_path = join(path, 'MG-LH')
    sym_path = join(path, 'MG-S')
    ...
    if not os.path.exists(speech_path):
        ...
        extract_melspec(audiopath, files, speech_path, fps, audio_format = '.mov.wav')
    else:
        action = input("'\ndel' or 'skip' existing speech features?\n")
        if action == 'del':
            os.removedirs(speech_path)
            os.makedirs(speech_path)
            ...
            print("Skipping speech features...")
        else:
            exit(-1)
    ...
    if not os.path.exists(motion_path):
        ...
        extract_joint_angles_saga(bvhpath, files, motion_path, fps)
    else:
        action = input("'\ndel' or 'skip' existing motion features?\n")
        if action == 'del':
            os.removedirs(motion_path)
            ...
            print("Skipping motion features...")
        else:
            exit(-1)
        
    # copy pipeline for converting motion features to bvh
    copyfile(join(motion_path, 'data_pipe.sav'), join(processed_dir,f'data_pipe_{fps}fps.sav'))
    ...
    jl.dump(input_scaler, join(processed_dir,f'input_scaler.sav'))         
    jl.dump(output_scaler, join(processed_dir,f'output_scaler.sav'))         
    np.savez(join(processed_dir,f'train_output_{fps}fps.npz'), clips = train_motion)
    np.savez(join(processed_dir,f'train_input_{fps}fps.npz'), clips = train_ctrl)
    np.savez(join(processed_dir,f'val_output_{fps}fps.npz'), clips = val_motion)
    np.savez(join(processed_dir,f'val_input_{fps}fps.npz'), clips = val_ctrl)
    np.savez(join(processed_dir,f'dev_output_{fps}fps.npz'), clips = dev_motion)
    np.savez(join(processed_dir,f'dev_input_{fps}fps.npz'), clips = dev_ctrl)
    np.savez(join(processed_dir,f'test_output_{fps}fps.npz'), clips = test_motion)
    np.savez(join(processed_dir,f'test_input_{fps}fps.npz'), clips = test_ctrl)

    # finally prepare data for visualisation, i.e. the dev and test data in wav and bvh format    
    dev_vispath = join(processed_dir, 'visualization_dev')
    test_vispath = join(processed_dir, 'visualization_test')    
    if not os.path.exists(dev_vispath):
        os.makedirs(dev_vispath)
        os.makedirs(test_vispath)
        
        for file in held_out:
            cut_audio(join(audiopath, file + ".mov.wav"), test_window_secs, dev_vispath, starttime=0.0,endtime=10*test_window_secs)
            cut_audio(join(audiopath, file + ".mov.wav"), test_window_secs, test_vispath, starttime=10*test_window_secs)
    else:
